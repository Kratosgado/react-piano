...
export const NATURAL_PITCH_INDICES: PitchIndex[] = [0, 2, 4, 5, 7, 8, 11]
...
export function generateNotes({ fromNote = LOWER_NOTE, toNote = HIGHER_NOTE }: NoteGeneratorSettings = {}): Note[] {
    return Array(toNote - fromNote + 1)
        .fill(0).map((_, index: number) => fromMidi(fromNote + index))
}
